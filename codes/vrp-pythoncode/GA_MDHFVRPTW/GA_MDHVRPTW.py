...
class Vehicle():
    def __init__(self):
        ...
        self.capacity=0
        self.free_speed=1
        self.fixed_cost=1.0 #车辆固定成本
        self.variable_cost=1.0 #车辆变动成本
        self.numbers=0
        self.start_time=0
        self.end_time=1440
...
#split过程采用标号法最短路思想，为了避免在搜索过程中产生大量劣质节点标签，
# 通过一定规则删除劣质标签：根据帕累托删除被支配的标签；根据剩余容量与剩余需求决定是否生成新标签。
#在搜索过程中需要计算可能车辆路径的成本（时间成本或距离成本），如果采用时间成本，这里为了简化，只计算旅行距离成本，忽略了等待时间成本。
# 但在计算适应度部分是严格按照时间成本内容计算的（旅行时间成本+等待时间成本）。
...
def checkTimeWindow(route,model,vehicle):
    ...
    for i in range(len(route)):
        if i == 0:
            ...
            departure = max(0, model.demand_dict[next_node_id].start_time - travel_time) #启程时间

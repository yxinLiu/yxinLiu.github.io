...
import csv
import sys #关于python运行环境
class Sol():
    def __init__(self):
        self.obj=None
        self.node_id_list=[] #需求节点id有序排列集合
        self.cost_of_distance=None #要花费的距离
        self.cost_of_time=None #时间成本
        self.fitness=None
        self.route_list=[]
        self.timetable_list=[] #车辆节点访问时间集合
...
class Node():
    def __init__(self):
        ...
        self.end_time=1440 #最晚结束服务时间
        self.service_time=0 #需求节点所需的服务时间

class Model():
    def __init__(self):
        self.best_sol=None
        self.demand_dict={} #需求节点集合，值类型为Node（）
        self.depot_dict={} #车场节点集合
        self.depot_id_list=[] #车场节点id集合
        self.demand_id_list=[] #需求节点id集合
        self.sol_list=[] #种群，类型为Sol（）
    ...
    for route in route_list:
        ...
        for i in range(len(route)):
            ...
            if i == 0:
                ...
                departure=max(0,model.demand_dict[next_node_id].start_time-travel_time) #到达要执行任务的节点所花费的路程时间
                timetable.append((departure,departure)) #用来做时间窗口

        ...
        while len(unEdgeList) > 0:
            ...
            for i in range(len(unEdgeList)):
                if unEdgeList[i].getDemand() <= timeList[0].vehicleLoad: # 有filter
                    ...
                    currentCVT = dis[timeList[1].vehiclePosition][head]
                    currentCVTF= currentCVT
                    currentCRC=dis[timeList[1].vehiclePosition][depot]
                    currentRQCV=timeList[1].vehicleLoad
                    currentRQCVF=currentRQCV
                    currentCRF=dis[timeList[1].vehiclePosition][depot]
                    for ve in range(2, len(timeList)):
                        if dis[timeList[ve].vehiclePosition][head] < currentCVT:
                        ...
                        if dis[timeList[ve].vehiclePosition][head] > currentCVTF:
                            currentCVTF = dis[timeList[ve].vehiclePosition][head]
                            currentRQCVF=timeList[ve].vehicleLoad
                            currentCRF=dis[timeList[ve].vehiclePosition][depot]
                    try:
                        hv = func(currentCFH,currentCR,currentCTD,currentCTT1,currentDEM,currentDEM1,currentFRT,currentFULL,currentRQ,currentSC)
                        hv2 = func2(currentCVT,currentCVTF,currentRQCV,currentRQCVF,currentDEM2,currentCTD2,currentCRC,currentCRF)
                        hv=0.9*hv+0.1*hv2
                    except:
                        print(individual)
                        print(hv)
...
def main():
    random.seed()
    # 设置种群数量
    pop = toolbox.population(n=820) #初始化了512个个体
    pop2 = toolbox2.population2(n=204)
    ...
    pop, pop2, log = gpSimple(pop, pop2, toolbox, toolbox2, 0.8, 0.15, 51, stats=mstats,

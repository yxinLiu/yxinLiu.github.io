    ...
    for batch in range(sampleNum):
        ...
        while len(unEdgeList) > 0:
            ...
            if len(candidateTask) == 0:
                ...
                # 遍历vehiculous种情况，确定chosenTaskID，体现协同任务分配的思想，使用分支定界算法求解指派问题的思想
                # chosenTaskID = candidateTask[0]
                row=len(timeList)
                col=len(candidateTask)
                assign=[[0]*col for i in range(row)]
                for r in range(row):
                    for c in range(col):
                        if unEdgeList[candidateTask[c]].getDemand()>timeList[r].vehicleLoad:
                            assign[r][c]=dis[timeList[r].vehiclePosition][depot]+dis[depot][unEdgeList[candidateTask[c]].getHead()]
                        else:
                            assign[r][c]=dis[timeList[r].vehiclePosition][unEdgeList[candidateTask[c]].getHead()]
                exchange=True
                for c in range(col-1):
                    if assign[0][c]<assign[0][c+1]:
                        exchange =False
                if exchange==True:
                    for r in range(row):
                        for c in range(col):
                            assign[r][c]=-assign[r][c]
                row_ind,col_ind=linear_sum_assignment(assign)
                chosenTaskID = candidateTask[col_ind[0]]
                '''
                col1=[]
                col2=[]
                for i in range(col):
                    minVehicle = 0
                    for j in range(row):
                        if(assign[j][i]<assign[minVehicle][i]):
                            minVehicle = j
                    col1.append(assign[minVehicle][i])
                    col2.append(minVehicle)

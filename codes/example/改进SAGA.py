...
def car_line_goods(population,dgoods_df,pgoods_df,dgoods_group_df,pgoods_group_df,dis_matrix,sa_num,count_i):
    ...
    while T>Tend:
        ...
        while (d <= len(population)-1) | (p <= len(population)-1):
            ...
            if (dvi_sum<=V-dv_sum) & (dmi_sum<=M-dm_sum) & (pvi_sum<=V-pv_sum) & (pmi_sum<=M-pm_sum):
                ...
                #是否跳出循环：货物装载完了/概率选择Pt/装的体积或质量正好等于车辆的但没超
...
def get_fit(populations, population_start, batch_size, dgoods_df,pgoods_df,dgoods_group_df,pgoods_group_df, dis_matrix, conn_send,sa_num):
    ...
    for i in range(batch_size):
        ...
        dcar_goods[i],pcar_goods[i],car_line[i],fit[i] = car_line_goods(populations[population_start+i].copy(),dgoods_df,pgoods_df,dgoods_group_df,pgoods_group_df,dis_matrix,sa_num,i)
        
    conn_send.send((car_line,fit,dcar_goods,pcar_goods))


def merchants_ga(dis_matrix,dgoods_df,pgoods_df,dgoods_group_df,pgoods_group_df,GeneSequence,ChromLength,generations,popsize,tournament_size,pc,pm,sa_num,cpus=4):
    #初始化种群
    ...
    for i in range(cpus):
        ...
        processlist.append(Process(target=get_fit, args=(populations, batch_size * i, batch_size, dgoods_df,pgoods_df,dgoods_group_df,pgoods_group_df,dis_matrix, conn_send,sa_num)))
        processlist[i].start()
    ...
    while iter < generations:
        ...
        for i in range(cpus):
            conn_recv, conn_send = Pipe()
            pipelist.append(conn_recv)
            processlist.append(Process(target=get_fit, args=(population_new, batch_size * i, batch_size, dgoods_df,pgoods_df,dgoods_group_df,pgoods_group_df, dis_matrix, conn_send,sa_num)))
            processlist[i].start()
...
if __name__ == '__main__':
    ...
    population,fit,car_line,dcar_goods,pcar_goods,populations,fits,car_lines,dcar_goods_all,pcar_goods_all,fits_list=merchants_ga(dis_matrix,dgoods_df,pgoods_df,dgoods_group_df,pgoods_group_df,GeneSequence,ChromLength,generations,popsize,tournament_size,pc,pm,sa_num)

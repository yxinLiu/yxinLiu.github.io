    ...
    best_fit = 10000
        ...
        while (d <= len(population)-1) | (p <= len(population)-1):
            ...
            if (dvi_sum<=V-dv_sum) & (dmi_sum<=M-dm_sum) & (pvi_sum<=V-pv_sum) & (pmi_sum<=M-pm_sum):
                ...
                #是否跳出循环
                if ((d== len(population)-1) & (p== len(population)-1)) | ((dv_sum==V) | (dm_sum==M)) | ((pv_sum==V) | (pm_sum==M)):
...
if __name__ == '__main__':
    ...
    #ga参数
    ChromLength = len(loc_list)   #染色体长度，客户点ZZZZZZZZ位置
    ...
    # dgoods_df_pac = package_GA.main(loc_list,dgoods_df,M,N,V)
    # pgoods_df_pac = package_GA.main(loc_list,pgoods_df,M,N,V)
    
    # dgoods_df = dgoods_df_pac[['配送点序号','商品序号','体积','重量']]
    # pgoods_df = pgoods_df_pac[['配送点序号','商品序号','体积','重量']]
    ...
    # print("population",population) 例：[3,2,6,5,7,1,8,4],最终服务客户的先后顺序
    # print("fit",fit) 最终适应度值，例如294.6
    # print("populations",populations) 最后一代40个个体服务客户的先后顺序
    # print("fits",fits) 最后一代的适应度值
    # print("car_lines",car_lines) 最后一代，40个车辆路线
    # print("送货all",dcar_goods_all) 最后一代，40个送货
